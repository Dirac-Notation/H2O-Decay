...
for dataset in ["winogrande", "piqa", "openbookqa", "arc_e", "mathqa"]:
    ...
    for layer in range(32):
        ...
        attention_path = os.path.join(dir_path, "npy", dataset, "no_pruning", f"{layer}.npy")

        ideal_mask = np.load(attention_path) # 1, 32, 25, 25
        
        for mask_name in mask_list.keys():
            mask_path = os.path.join(dir_path, "npy", dataset, mask_name, f"{layer}.npy")
    ...
    folder_path = os.path.join(dir_path, "similarity")
